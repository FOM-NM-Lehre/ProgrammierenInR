...
Für uns als Anfänger:in sind die unterschiedlichen Datentypen (`[tibble](https://tibble.tidyverse.org)`, `data.frame`, `[data.table](https://rdatatable.gitlab.io/data.table/)`, `tbl`, ..) 
(annähernd) gleich zu benutzen. 
Auch wenn die Implementierungen sich teilweise starkt unterscheiden.
...
- `summarise()` reduziert mehrere Werte auf eine einzelne Zusammenfassung.
...
Neben der Arbeit mit Data Frames bzw. Tibbles ermöglicht `dplyr` auch einen effizienten und benutzerfreundlichen Umgang mit verschiedenen Backends. 
...
3. Nutzen Sie die Hilfe um die Ausgabe der Größe der Rechnungsbeträge nach zu sortieren! (Tipp: Ist etwas schwerer! Suchen Sie mit der Hilfe im Paket `dplyr` nach dem Befehl `arrange()`. ;-) ) 
