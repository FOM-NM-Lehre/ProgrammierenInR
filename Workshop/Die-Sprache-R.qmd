...
- 2007: Veröffentlichung von ggplot2, einem Paket basierend auf der [Grammar of Graphics](https://doi.org/10.1007/0-387-28695-0) von [Leland Wilkinson](https://en.wikipedia.org/wiki/Leland_Wilkinson), entwickelt von [Hadley Wickham](https://en.wikipedia.org/wiki/Hadley_Wickham). 
...
- 2014: Das Paket [dplyr](https://dplyr.tidyverse.) mit seinen modernen Datenmanipulationsfunktionen wurde veröffentlicht. Eines der einflussreichsten Pakete im R-Ökosystem.
...
Wir beginnen in dem wir uns in *posit.cloud* ein _R-Skript_ Dokument erstellen und gemeinsam einige Schritte durch gehen um unser erstes (gemeinsames) _R_ zu programmieren.
...
## Unterschied zum Standard-Datenrahmen
...
```{webr-r}
#| autorun: true
# Blick auf den Datenrahmen
glimpse(tib)
```
:::
:::

Für Anfänger:in sind die unterschiedlichen Datentypen ([`tibble`](https://tibble.tidyverse.org), `data.frame`, [`data.table`](https://rdatatable.gitlab.io/data.table/), `tbl`, ...) 
(annähernd) gleich zu benutzen. 
Auch wenn die Implementierungen sich teilweise starkt unterscheiden und es kontekt dann vor- bzw. nachteile geben kann.
...
1. Erzeugen Sie eine neue Tabelle mit dem Namen *tib2*, welche die Spalten
    _Groesse_, _Gewicht_ und _BMI_ enthält.
...
[`dplyr`](http://dplyr.tidyverse.org) ist eine _Grammatik zur Datenmanipulation_ und bietet eine konsistente Sammlung von Verben, die helfen, die häufigsten Herausforderungen bei der Datenbearbeitung zu lösen. 

Ein Überblick:

- [`mutate()`](https://dplyr.tidyverse.org/reference/mutate.html) fügt neue Variablen hinzu, die Funktionen bestehender Variablen sind.
- [`select()`](https://dplyr.tidyverse.org/reference/select.html) wählt Variablen basierend auf ihren Namen aus.
- [`filter()`](https://dplyr.tidyverse.org/reference/filter.html) wählt Fälle basierend auf ihren Werten aus.
- [`summarise()`](https://dplyr.tidyverse.org/reference/summarise.html) reduziert mehrere Werte auf eine einzelne Zusammenfassung.
- [`arrange()`](https://dplyr.tidyverse.org/reference/arrange.html) verändert die Reihenfolge der Zeilen.

Diese Funktionen lassen sich mit [`group_by()`](https://dplyr.tidyverse.org/reference/group_by.html) kombinieren, wodurch jede Operation „nach Gruppe“ ausgeführt werden kann.


## Backends zu `dplyr`

Neben der Arbeit mit Data Frames bzw. Tibbles ermöglicht [`dplyr`](http://dplyr.tidyverse.org) auch einen effizienten und benutzerfreundlichen Umgang mit verschiedenen Backends. 
Nachfolgend dine Übersicht alternativer Backends:

- [**arrow**](https://arrow.apache.org/docs/r/): Für Datensätze, die größer sind als der verfügbare Arbeitsspeicher – einschließlich solcher, die in entfernten Cloud-Speichern wie AWS S3 abgelegt sind.

- [**dtplyr**](https://dtplyr.tidyverse.org): Für große Datensätze im Arbeitsspeicher. Übersetzt `dplyr`-Code in leistungsstarken [`data.table`](https://rdatatable.gitlab.io/data.table/)-Code.

- [**dbplyr**](https://dbplyr.tidyverse.org): Für Daten, die in relationalen Datenbanken gespeichert sind. Wandelt `dplyr`-Anweisungen in SQL-Abfragen um.

- [**duckplyr**](https://duckplyr.tidyverse.org): Für die Nutzung von _duckdb_ bei großen In-Memory-Datensätzen ohne zusätzliche Kopien. Übersetzt `dplyr`-Code in effiziente *duckdb*-Abfragen und greift bei nicht möglicher Übersetzung automatisch auf R zurück.

- [**duckdb**](https://duckdb.org/docs/stable/clients/r.html): Für große Datensätze, die dennoch vollständig auf einem lokalen Rechner verarbeitet werden können.

- [**sparklyr**](https://spark.posit.co): Für sehr große Datensätze, die in [Apache Spark](https://spark.apache.org) gespeichert sind.


## Spalten zu Tabellen hinzufügen mit  `mutate()`

Wir nutzen dazu das Paket [`dplyr`](http://dplyr.tidyverse.org) und den Befehl `mutate()`:
...
## Ein Boxplot in zwei Dimensionen
...
1. Nutzen Sie die Tabelle *tib3* und erstellen Sie einen Boxplot für die Varable *Größe*.

2. Nutzen Sie die Tabelle *tib3* und erstellen Sie einen Boxplot für die Variable *Größe*, aber nach Geschlechtern getrennt!


## Streudiagramme

Zur Darstellung von (vermuteten, linearen) Zusammenhängen zweier numerischer Variablen nutzen wir _Streudiagramme_ (engl. _Scatterplot_, hier mit Hilfe des Geoms: `geom_point()`).
...
## Eine Grafik statt nur die Kennzahlen
...
## Spalten und Zeilen auswählen mit `select()` und `filter()` aus `dplyr`
...
## Zusammenfassungen von Daten mit `summerise()`
...
Um Daten aus einer Excel-Datei zu lesen brauchen wir das Paket [`readxl`](https://readxl.tidyverse.org).
...
1. Versuchen Sie aus der Tabelle *tips_csv* die Anzahl der Frauen die Zahlen. \
    **Tipp:** _`sex == "Female"`) und deren Rechnungsbeträge (Tipp: `total_bill`) auszugeben_


2. Gruppieren Sie nun die Ausgabe aus Aufgabe 1. so, dass sie nach Tagen ausgegeben wird. \
    **Tipp:** _`days`_


3. Nutzen Sie die Hilfe um die Ausgabe der Größe der Rechnungsbeträge nach zu sortieren! \
    a) Einmal in aufsteigender Reihenfolge!
    
    b) Einmal in absteigender Reihenfolge!
    
    **Tipp:** _Ist etwas schwerer! Suchen Sie mit der Hilfe im Paket [`dplyr`](https://dplyr.tidyverse.org/) nach dem Befehl [`arrange()`](https://dplyr.tidyverse.org/reference/arrange.html)._
...
Wir können in _R_ zwei Zahlen miteinander vergleichen und feststellen, ob diese gleich oder ungleich sind.
Wir können auch überprüfen, ob eine Zahl größer oder kleiner ist als eine andere Zahl.
Eine solche Frage wird dann in _R_ mit entweder _Ja_ (`TRUE`) oder _Nein_ (`FALSE`) beantwortet. 
